<template>
	<div class="card mb-3">
		<div class="card-body">
			...
			<a v-if="!is_bookm" @click="add_bookmark">
				...
				<a v-if="is_bookm" @click="remove_bookmark">
				<svg xmlns="http://www.w3.org/2000/svg"  :width="icon_width+'px'" :height="icon_hegiht+'px'" fill="currentColor" class="bi bi-bookmark float-right book_mark_logo" viewBox="0 0 16 16"   v-if="is_bookm">
			...
			<a v-if="!$parent.is_already_liked" @click="add_like">
			...
			<a v-if="$parent.is_already_liked" @click="remove_like">
			...
			<router-link :to="'/user/post/view/' + post_id">
...
import axios from 'axios'
	...
	created(){
		...
		this.bookmark_api_path = process.env.VUE_APP_FLASK_SERVER_URL + "/api/v2/bookmark/post/" + window.localStorage.getItem('user_id') + "/"  + this.post_id
